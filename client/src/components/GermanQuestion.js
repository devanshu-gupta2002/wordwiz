...
const GermanQuestion = () => {
  ...
  useEffect(() => {
    ...
    if(loggedUserData){
      ...
      questionServices.getAll().then((response) => setQuestions(response.filter((question) => question.language === "German")))
      }
    else {
      navigate("/")
    }
